...
template <>
Vector3d CheckJson<Vector3d>(nlohmann::json j, std::string str1, std::string str2){
	if(j[str1][str2].is_null()){
		std::cout << "Invalied parse: " << str1 << std::endl;
	}
	return Vector3d{ j[str1][str2][0].get<double>(), j[str1][str2][1].get<double>(), j[str1][str2][2].get<double>() };
}

template <>
Vector3d CheckJson<Vector3d>(nlohmann::json j, std::string str1){
	if(j[str1].is_null()){
		std::cout << "Invalied parse: " << str1 << std::endl;
	}
	return Vector3d{ j[str1][0].get<double>(), j[str1][1].get<double>(), j[str1][2].get<double>() };
}
...
inline void readJson(const std::string json_file, MPM::MaterialParameters& mat_params, MPM::SimulationParameters& sim_params, MPM::SituationSets& situation_set)
{
	...
	sim_params.box_min = CheckJson<Vector3d>(j,"SimSetting","box_min");
	
	std::cout << "-> box_max	: " << 	j["SimSetting"]["box_max"] 		<< std::endl;
	sim_params.box_max = CheckJson<Vector3d>(j,"SimSetting","box_max");
	...
	std::cout << "BoundaryFromSDF" << std::endl;
	for (auto& element : j["BoundaryFromSDF"]) {
		std::string filename = CheckJson<std::string>(element,"filename");
		situation_set.boundary_conditions.rigidbodies.push_back(MPM::RigidBody{filename});
	}
	for (auto& e : situation_set.boundary_conditions.rigidbodies) {
		std::cout << "RigidBody SDF Filename : " << e.m_filename << std::endl;
